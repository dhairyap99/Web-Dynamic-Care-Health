...
<body>
<div class="title">
		<h2 style="font-family: Verdana">Reschedule Appointment</h2>
		<hr>
	</div>
<table cellpadding="6">
...
rs=st.executeQuery("SELECT booking.bookingid,users.fname,users.lname,booking.date,booking.time,booking.status,doctordetails.location "
		...
		+"WHERE booking.pname='"+uname+"'and booking.date>'"+today+"' and booking.status=1 and booking.time>NOW() "
		+"ORDER BY booking.date");
while(rs.next()){
	out.println("<tr>");
	out.println("<td><a href=\"AppointmentResch1.jsp?bid="+rs.getString(1)+"\">"+rs.getString(1)+"</a></td>");
